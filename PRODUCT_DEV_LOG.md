...
## 2026-05-06 14:05:13 | 本地与子域名页面不一致排查
- 问题现象: 用户发现本地网址打开后的内容与 `manim.yishuziyu.cn` 不一致，怀疑是否导入了错误的 GitHub 仓库或本地项目目录。
- 解决方案: 先用 `git remote -v` 和 `git status -sb` 确认当前目录仍是 `https://github.com/yishu-ziyu/Aegis-Manim.git` 的 `main` 分支；再检查端口监听，发现 `127.0.0.1:8000` 被另一个 `uvicorn curiocat.main:app` 进程占用，`./scripts/web_server.sh status` 显示 Aegis Web 未运行。随后将本地 Aegis Web 启动到 `http://127.0.0.1:8010` 验证，并把 Vercel `api/index.py` 改为复用 `core/web_app.py` 的 Aegis Studio HTML，只在 gateway 层替换云端模式文案、禁用本机渲染 checkbox、隐藏本机专用 Provider。
...
## 2026-09-09 03:08:00 | 夜航 UI 落地 + 模拟用户 QA 飞轮首轮
- 问题现象: 旧 UI（米色纸感 + 衬线全站 + 第一屏六项技术配置）与产品定位脱节，用户反馈"外观丑、引导语糟糕"；生成完成无下载入口、进度步骤终态不归位、Instant SVG 四大工作流之一在 Web 端 404 断链、生成的福利区域动画存在经济学几何错误（损失的剩余只画了转移矩形的一半）。
- 解决方案: 按 docs/ui-redesign-spec.md（温润手帐风 × Apple Design，参考 ManimCat tokens + apple-design/emil-design-eng skill）重做界面：暗色默认 #17171b + 亮/暗切换、落地舞台→工作台双状态、示例 prompt 卡与话题缩略图引导层、六项配置折叠进高级设置、右栏 4 Tabs、霞鹜文楷字体。QA 飞轮按 EVAL 原理模拟真实用户（5 persona × 7 scenario）在浏览器里跑完整路径，共修复 12+ findings：下载视频按钮、进度步骤终态归位+面板自动收起、成功文案中文化、Instant SVG 路由+顶栏入口+返回链接、仓库搜索失败中文化并诚实停用、亮色 accent-2 对比度 2.1→4.5+、正文 14→15px、prompt 2000 字上限+计数器、缩略图行为与文案一致、视频空态三段生命周期、渲染清晰度选择（480p/720p 默认/1080p）。
- 方案原因: 用户明确要求"以真实用户模拟 QA"的持续迭代方式（参考 eval skill 底层原理）；静态 DOM 检查会漏掉交互后的动态状态（暗色白色块、进度终态、空态都是真实操作才暴露的）。知识包层面的经济学区域几何错误只有靠审查生成产物（代码+逐帧看片）才能发现。
- 下次识别与避免: 改 f-string 内嵌 HTML/CSS/JS 时先用脚本做唯一性断言替换再 py_compile；grid 子项必须 min-width:0；亮暗双主题的每个组件都要分别截图验证；知识包修改后要用同题生成做 A/B 对比验证行为变化。
- 结果与经验: 默认链路（Token Plan 调 MiniMax M3、720p 渲染、中文引导）全通；知识包 get_area/福利区域规则经同题对比验证已改变生成行为；发现 .env 从未被加载的隐藏 bug 并修复；本地未部署渲染后端时仓库功能诚实降级。所有改动未提交 git，待用户验收。

## 2026-09-09 04:30:00 | 4 小时 QA 飞轮后程（R31-R43）：生成质量规则体系成型
- 问题现象: 飞轮后半程连续审阅真实生成产物（代码+逐帧看片），暴露出 prompt 规则层的系统性弱点——底部结论裁切重叠（F-029）、均衡点坐标手打字面量算错致点悬空在交点外（F-030/F-033/F-036/F-039 四次复发）、总结声明超出演示范围（F-031）、移动端下载按钮 28px 触控不达标（F-032）、政策幅度与声明不匹配（F-034）、多曲线角部标签重叠（F-035/F-038）、矩形标注撞标题（F-037）。
- 解决方案: 13 轮真实生成（IS-LM/供需/外部性×3/弹性×2/PPF/菲利普斯×2）逐帧审阅，知识包新增 8 条规则（均衡点必须在 Python 内由参数推导禁止字面量、底部总结硬布局、总结只声明演示内容、政策幅度按声明校准、角部标签显式堆叠、面积标注放矩形内部等），其中"数值正确性"因知识包采纳率仅 2/3 而前移到 system_prompt.md 的 Constraints (CRITICAL) 第 9 条（BAD/GOOD 代码对照）——前移当轮即生效。另修 UI：960px 断点触控目标 min-height:44px。
- 方案原因: 生成质量问题的修复闭环必须是"审阅产物→改规则→同题重生成→视频逐帧验证"；字面量坐标错误靠提示词约束只能提高概率，把算术交给解释器（代码内计算）才是根因级修复；重复出现的失误家族要前移到更高权重的指令位而不是反复同题重生成。
- 下次识别与避免: 知识包规则要用"模型可抄的代码模式"而非抽象描述；审阅生成视频时优先 zoom 均衡点/角部标签/底部字幕三个高风险区；移动端回归要真设 390×844 视口（tab.setViewportSize）而非只看响应式 CSS。
- 结果与经验: 11 个 finding 全部闭环，生成质量肉眼可见地逐轮提升（最后一轮弹性动画自发用双色叠加+黄框标注损失/获得面积，教学表达超出预期）；桌面+移动端 UI 回归全过（Tab 互斥、空输入守卫、主题持久化、44px 触控目标）。台账见 docs/qa-loop-20260909.md。改动未提交 git，待用户验收。
